...
public final class ParBoruvka extends AbstractBoruvka<ParBoruvka.ParComponent> {
    ...
    private ParComponent[] result = new ParComponent[1];
    ...
    public void computeBoruvka(final Queue<ParComponent> nodesLoaded,
        ...
        while ((n = nodesLoaded.poll()) != null) {
            ...
            // Safe to parallelize two iterations working on disjoint pairs
            if (!n.lock.tryLock()) {
                continue;
            }

            // release lock if n has already been merged
            if (n.isDead) {
            n.lock.unlock();
            continue;
            }
            ...
            // if e == null, only one component remains
            if (e == null) {
            // edges are sorted, so index 0 is min weighted edge
            result[0] = n;
            ...
            // release first lock if cannot lock for second vertex
            ...
            // release locks if other component already merged
            if (other.isDead) {
            other.lock.unlock();
            n.lock.unlock();
            nodesLoaded.add(n);
    ...
    public static final class ParComponent extends Component<ParComponent> {
         ...
         * TODO : Maybe switch array list to linked list here since
         * add.(i,e) isn't very efficient...(is always O(n))
         */
