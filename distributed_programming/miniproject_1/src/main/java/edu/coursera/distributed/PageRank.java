...
public final class PageRank {
    ...
    public static JavaPairRDD<Integer, Double> sparkPageRank(
        ...
        JavaPairRDD<Integer, Double> newRanks =
                sites.join(ranks).// both site's and rank's have ids --> can join db's
                        // map to get contributions of each site
                        flatMapToPair( kv -> {
                            Integer websiteId = kv._1(); // extract id from key
                            Tuple2<Website, Double> value = kv._2(); // extract value
                            Website edges = kv._2()._1(); // get edges from value
                            Double currentRank = kv._2()._2(); // extract the current rank

                            // Accumulate the new contributions to each rank
                            // List of contributions for each edge.
                            List<Tuple2<Integer, Double>> contributions =
                                    new LinkedList<>();
                            Iterator<Integer> iter = edges.edgeIterator();
                            while( iter.hasNext() ){
                                final int target = iter.next();
                                contributions.add(new Tuple2<>(target,
        ...
        // the 'reduce step'
        // note: the reduce operation often limits the effects of parallelism
