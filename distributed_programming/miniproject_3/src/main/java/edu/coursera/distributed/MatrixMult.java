...
public class MatrixMult {
    ...
    public static void parallelMatrixMultiply(Matrix a, Matrix b, Matrix c,
        ...
        final int myRank = mpi.MPI_Comm_rank(mpi.MPI_COMM_WORLD); // get the rank of current process
        final int size   = mpi.MPI_Comm_size(mpi.MPI_COMM_WORLD); // get the number of ranks we are working with

        final int nRows = c.getNRows();
        final int rowChunk = (nRows + size -1) / size; // chunk up each row to enhance parallelism (1 chunk/process)
        final int startRow = myRank*rowChunk;
        int       endRow   = (myRank + 1) * rowChunk;
        if (endRow > nRows) endRow = nRows;

        // broadcast the matrices from rank 0 to all other ranks
        // note: for very large matrices, better to slice up the matrix to avoid having each
        // proccess  holding the large data in memory
