    ...
    public static void runParallelFuzzyBarrier(final int iterations,
        ...
        for(int ii=0; ii < tasks; ii++){
            ...
            threads[ii] = new Thread(()->{
                ...
                for(int iter = 0; iter < iterations; iter++){
                    // the following boundaries define a chunk
                    // left boundary
                    ...
                    // at this point, the thread is signalling it had completed some shared
                    // work of interest to other tasks (it's boundary elements) and proceeds
                    // to do some local work (that does not depend on any other tasks)
                    ...
                    // wait for previous phase to complete before advancing to next iteration
